...
class TrainingDaysKeyboard(InlineKeyboardBuilder):
    def __init__(self, days: list, target: str, go_back_target: str, lang):
        super().__init__()
        for idx, day in enumerate(days):
            self.row(InlineKeyboardButton(text=translations[lang].trainer_client_plan_days_keyboard_day_btn.value.format(idx+1, day.name),
...
class TrainingDayExercises(InlineKeyboardBuilder):
    def __init__(self, exercises: list, target: str, go_back_target: str, lang):
        super().__init__()
...
class PlanExerciseGoBackKeyboard(InlineKeyboardBuilder):
    def __init__(self, source_option, go_back_target: str, lang):
        super().__init__()
