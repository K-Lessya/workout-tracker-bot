...
@add_exercise_router.callback_query(ChooseCallback.filter(F.target == ListBodyPartsTargets.add_exercise))
async def add_exercise(callback: CallbackQuery, callback_data: ChooseCallback, state: FSMContext):
    choosed_option = callback_data.option
    if choosed_option == ListBodyPartsOptions.add_pure_exercise:
        await state.set_state(TrainerStates.add_exercise.process_body_part_name)
        await callback.message.edit_text(f'Введи название части тела на которую делается упражнение')


@add_exercise_router.message(TrainerStates.add_exercise.process_body_part_name)
async def process_bodypart_name(message: Message, state: FSMContext):
    await state.update_data({'body_part': message.text})
    await state.set_state(TrainerStates.add_exercise.process_muscle_group_name)
    await message.answer(f'Теперь введи группу мыщц для которой предназначено это упражнение')


@add_exercise_router.message(TrainerStates.add_exercise.process_muscle_group_name)
async def process_muscle_group_name(message: Message, state: FSMContext):
    await state.update_data({'muscle_group': message.text})
    await state.set_state(TrainerStates.add_exercise.process_exercise_name)
    await message.answer(f'А теперь введи название упражнения')


@add_exercise_router.message(TrainerStates.add_exercise.process_exercise_name)
async def process_muscle_group_name(message: Message, state: FSMContext):
    await state.update_data({'exercise_name': message.text})
    await state.set_state(TrainerStates.add_exercise.process_photo)
...
async def ask_for_photo(callback: CallbackQuery, callback_data: ChooseCallback, state: FSMContext):
    if callback_data.option == YesNoOptions.yes:
        ...
        await state.update_data({'exercise_photo_link': 'defaults/no-exercise-photo'})
        await state.set_state(TrainerStates.add_exercise.process_video)
...
@add_exercise_router.message(TrainerStates.add_exercise.process_photo, F.photo)
...
@add_exercise_router.message(TrainerStates.add_exercise.process_video)
...
async def proces_save(callback: CallbackQuery, callback_data: ChooseCallback, state: FSMContext):
    if callback_data.option == YesNoOptions.yes:
        ...
        if state_data['exercise_photo_link'] != 'defaults/no-exercise-photo':
        ...
        await callback.message.edit_text(f'Упражнение не сохранено, возвращаемся в меню упражнений',
                                         reply_markup=create_show_body_parts_keyboard(body_parts))
        await callback.answer()
